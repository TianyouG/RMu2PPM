...
def _sampling_worker(graph_shared):
    ...
    node_weights_shm = shm.SharedMemory(name=graph_shared["node_weights_shm"])
    ...
    node_weights = np.ndarray(
        (node_weights_shm.size // np.dtype(np.float64).itemsize,),
        dtype=np.float64,
        buffer=node_weights_shm.buf
    )
    ...
    v = rng.choice(len(node_weights), p=node_weights)
    activated[v] = True
...
def imm_cbga_sampling(graph_shared, k, epsilon, delta):
    ...
    node_weights = comm_sizes[communities]
    log_node_weights = np.log1p(node_weights)
    log_node_weights = np.maximum(log_node_weights, 0)
    min_nonzero = np.min(log_node_weights[log_node_weights > 0])
    log_node_weights[log_node_weights == 0] = min_nonzero
    total_log_weight = np.sum(log_node_weights)
    node_weights = log_node_weights / total_log_weight
    node_weights = np.maximum(node_weights, 0)
    node_weights[np.isnan(node_weights)] = 0
    node_weights = np.array(node_weights, dtype=np.float64)
    node_weights = node_weights / np.sum(node_weights)
    logging.info("Final node weights sum: {:.6f}".format(np.sum(node_weights)))
    logging.info("Total log weights: {}, node_weights (first 5): {}".format(total_log_weight, node_weights[:5]))
    node_weights_shm = shm.SharedMemory(create=True, size=node_weights.nbytes)
    np.ndarray(node_weights.shape, dtype=node_weights.dtype, buffer=node_weights_shm.buf)[:] = np.copy(node_weights)
    graph_shared["node_weights_shm"] = node_weights_shm.name
    ...
    RR_set = parallel_generate_rr_sets(graph_shared, theta_0 - theta[-1])
    node_to_rr = defaultdict(list)
    c = np.zeros(n, dtype=np.int32)
    ...
    node_weights_shm.close()
    node_weights_shm.unlink()
    return RR_set, c, node_to_rr


def imm_weighted_optimized(graph_shared, graph_name, k, epsilon, delta, setR=10000):
    logging.info("Start IMM_weighted, graph:{}, P1={}, k={}, epsilon={}, delta={}".format(graph_name, P1, k, epsilon, delta))
    ...
    logging.info("IMM_weighted Finished. Sampling Time: {:.2f}s, Running Time: {:.2f}s".format(sampling_time, running_time))
